...
class CreditCardController extends Controller
{
    ...
    public function cardTopup(Request $request)
    {
        ...
        $creditCard = CreditCard::findOrFail($card_id);
        $prepaidcard = Card::findOrFail($creditCard->card_id);
        ...
        $limitFee = $prepaidcard->increase_limit_fee;
        $depositAmount = ($amount * $limitFee) / 100;
        ...
        if($prepaidcard->topup_limit < $amount){
            Session::flash('flash', [
                'type' => 'error',
                'message' => __('Maximum card topup is '.$prepaidcard->topup_limit),
            ]);
        ...
        $cardHelper = new CardHelper();
        $cardUsesLimit = $cardHelper->cardTopup($card_id, $amount);

        User::where('id', $user_id)->update(['balance' => $user->balance - $totalAmount]);    
        ...
        $topUp->type                = 'balance';
        $topUp->order_uid           = null;
        $topUp->transaction_fee     = $totalAmount - $amount;
        $topUp->rate                = 1;
        $topUp->requested_amount    = $amount;
        $topUp->meta                = null;
        $topUp->save();

        $topupId = $topUp->id;

        CreditCard::where('id', $card_id)->update(['available_limits' => $creditCard->available_limits + $amount]);
        ...
        $template_sub = $template->subject;
        $template_msg = str_replace('{user}', $user->first_name . ' ' . $user->last_name, $template->body);
        $template_msg = str_replace('{card}', $creditCard->last4, $template_msg); 
        $template_msg = str_replace('{amount}', $totalAmount, $template_msg);
        $template_msg = str_replace('{date}', now(), $template_msg); 
        ...
        return redirect()->route('user.credit-cards.show', $card_id)->with('flash', [
            'type' => 'success',
            'message' => 'Card topup done successfully'
        ]);
    }
