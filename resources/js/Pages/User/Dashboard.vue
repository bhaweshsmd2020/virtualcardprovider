...
function submitTopup() {
  if (!form.value.card_id || !form.value.amount) {
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-2">
        ...
        <section class="md:col-span-3 flex flex-col space-y-2">   
          ...
          <div class="flex bg-white justify-between border border-gray-200 rounded-xl p-3 mb-6 gap-4">
            <button @click="showTopupModal = true" class="flex flex-col items-center text-gray-500 text-xs space-y-1" type="button">
            ...
            <div v-if="showTopupModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" @click.self="closeModal" style="z-index: 99;">
              <div class="bg-white p-6 rounded-xl w-full max-w-md" style="max-width: 690px;">
                <div class="flex justify-between items-center mb-4">
                  <h2 class="text-lg font-semibold">Card Topup</h2>
                  <button @click="closeModal" class="text-dark-500 hover:text-dark-800 text-3xl font-bold focus:outline-none">
                    &times;
                  </button>
                </div>
                <form @submit.prevent="submitTopup">
                  <div class="mb-4">
                    ...
                    <select v-model="form.card_id" required class="w-full border border-gray-300 rounded p-2">
                      <option disabled value="">Select a card</option>
                      <option v-for="card in creditCards" :key="card.id" :value="card.id">
                        {{ card.name_on_card }} - **** **** **** {{ card.last4 }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div class="mb-4">
                    ...
                    <input type="number" v-model="form.amount" required class="w-full border border-gray-300 rounded p-2" placeholder="100"/>
                  </div>

                  <div class="my-4 grid grid-cols-4 gap-2">
                    <button type="button" class="relative rounded border dark:border-gray-600">
                  ...
                  <div>
                    <label class="label font-semibold dark:text-gray-200">
                      {{ trans('Payment Instruction: ') }}
                    </label>
                    <p class="mt-1 text-sm dark:text-gray-200 payment-inst">Virtual Card Provider</p>
                  </div>                  

                  <div class="flex justify-end space-x-2">
                    <button type="submit" class="btn btn-primary mt-4 w-full py-2 text-lg">Pay Now</button>
                  </div>
                </form>
              </div>
            </div>
          ...
          <div class="bg-white rounded-xl p-4 shadow-sm flex flex-col space-y-2">
            <div class="order-2 col-span-1 md:col-span-7 xl:order-3">
              <div class="card-body flex h-full flex-col justify-between gap-2">
                <IncomeChart :key="incomeChartKey" />                  
              </div>
            </div>
          </div> 
          ...
          <div class="card lg:col-span-12">
            <div class="card-body space-y-2">
              <div class="flex justify-between items-center text-sm font-bold text-[#1E293B]">
                ...
                <Link :href="route('user.top-up.index')" class="detail-list">
                  All Topups
                </Link>
              </div>
                ...
                <table class="table">
                  ...
                  <tbody v-if="recentTopUps.length > 0">
                    <tr v-for="topUp in recentTopUps" :key="topUp.id">
                      ...
                      <td>
                        <span v-if="topUp.type === 'balance'">Account</span>
      ...
      <section class="grid grid-cols-1 place-items-start gap-2 lg:grid-cols-12">
        <div class="card lg:col-span-7">
          <div class="card-body">
            ...
            <div class="table-responsive whitespace-nowrap rounded-primary" style="height: 245px; overflow-y: scroll;">
              <table class="table">
                <thead>
                  <tr>
