...
<template>
  ...
  <main class="container flex-grow p-4 sm:p-6">
    ...
    <div class="space-y-4">
      <p class="text-lg font-semibold">Authorization History</p>
        ...
        <table class="table">
          ...
          <tbody v-if="authorizations?.data?.length > 0">
            <tr v-for="authorization in authorizations.data" :key="authorization.id">
              ...
              <td>
                {{ authorization.card.cardholder.phone_number || 'None' }}
              </td>
      ...
      <StripePagination :has_prev="prev" :has_more="has_more" :prev="prev" :next="next" v-if="next != ''" />
    </div>
  </main>
</template>
