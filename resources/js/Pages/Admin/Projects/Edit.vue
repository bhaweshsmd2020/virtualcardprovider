...
const props = defineProps(['buttons', 'project', 'categories'])
const project = props.project
const form = useForm({
  category_id: project.category_id ?? '',
  title: project.title ?? '',
  slug: project.slug ?? '',
  release_at: project.release_at ? moment(project.release_at).format('YYYY-MM-DD') : '',
  preview: '',
  banner_preview: '',
...
const submit = () => {
  form.post(route('admin.projects.update', props.project), {
    onSuccess: () => {
      form.reset()
    }
  })
...
<template>
  <main class="container p-4 sm:p-6">
    <PageHeader title="Edit Project" :buttons="buttons" />
      ...
      <div class="card-body">
        <form @submit.prevent="submit">
          <div class="grid grid-cols-2 gap-x-2">
            ...
            <div class="mb-2">
              <label class="label mb-2">{{ trans('Release Date') }}</label>
              <input type="date" v-model="form.release_at" class="input" />
              <InputFieldError :message="form.errors.release_at" />
            </div>
