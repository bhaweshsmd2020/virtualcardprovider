    ...
    <div class="cookies__alert">
        <div class="cookies__container">
            <div class="cookies__wrapper">
                <h5 class="cookies__title">{{ $cookieData['title'] }}</h5>
                <div class="cookies__intro">
                    <p>{{ $cookieData['intro'] }}</p>
                    @if($policy)
                    <p>@lang('cookieConsent::cookies.link', ['url' => $policy])</p>
                    @endif
                </div>
                <div class="cookies__actions">
                    @cookieconsentbutton(action: 'accept.essentials', label: $cookieData['essentials'],
                    attributes: ['class' => 'cookiesBtn cookiesBtn--essentials'])
                    @cookieconsentbutton(action: 'accept.all', label: $cookieData['all'], attributes:
